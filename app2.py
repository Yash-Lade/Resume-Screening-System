...
def extract_certifications(text, certification_list):
...
def rank_resumes(job_id):
    ...
    # Extract required experience from job description (default to 0 if not specified)
    required_experience = extract_experience(job['description'])
    ...
    for app in job_applications:
        ...
        # Extract years of experience
        experience_years = extract_experience(app['resume_text'])

        # Extract certifications
        certifications = extract_certifications(app['resume_text'], COMMON_CERTIFICATIONS)

        # Calculate skills match score (30% weight)
        ...
        # If required experience is 0, give full score for any experience
        if required_experience == 0:
            experience_score = 1.0 if experience_years > 0 else 0.5
        else:
            # Score based on ratio of candidate's experience to required experience
            # Cap at 1.0 for candidates with more than required experience
            experience_score = min(1.0, experience_years / required_experience) if required_experience > 0 else 0.5

        # Calculate certifications score (10% weight)
        # Give points for having relevant certifications
        certifications_score = min(1.0, len(certifications) / 3)  # Cap at 3 certifications for full score
        ...
        skills_score_scaled = max(1, min(100, round(skills_score * 100)))
        similarity_score_scaled = max(1, min(100, round(similarity_score * 100)))
        ...
        app['skills_score'] = skills_score_scaled
        app['similarity_score'] = similarity_score_scaled
    ...
    for i, app in enumerate(ranked_applications):
        with st.expander(f"{i+1}. {app['candidate_name']} - Match Score: {app.get('overall_score', 0)}/100"):
            ...
            with col1:
                ...
                st.write(f"**Skills Match:** {app.get('skills_score', 0)}/100")
                st.write(f"**Content Similarity:** {app.get('similarity_score', 0)}/100")
                st.write(f"**Experience Score:** {app.get('experience_score', 0)}/100")
                st.write(f"**Certifications Score:** {app.get('certifications_score', 0)}/100")
                ...
                if 'certifications' in app and app['certifications']:
                    st.write(f"**Certifications Found:** {', '.join(app['certifications'])}")
